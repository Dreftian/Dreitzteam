...
interface SteamAppDetails {
  ...
  price_overview?: { initial: number; final: number; discount_percent: number; currency: string };
...
function fetchJson(url: string): Promise<any> {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'Accept-Language': 'es-PE,es;q=0.9' } }, (res) => {
      let data = '';
...
export function extractAppIdFromUrl(input: string): number | null {
  const t = input.trim();
  const m = t.match(/store\.steampowered\.com\/app\/(\d+)/i);
...
export async function fetchSteamPriceUsd(appId: number): Promise<{ priceUsd: number | null; discount: number | null }> {
  try {
    ...
    return { priceUsd: entry.data.price_overview.final / 100, discount: entry.data.price_overview.discount_percent ?? 0 };
  } catch {
